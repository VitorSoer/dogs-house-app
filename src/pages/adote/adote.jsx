...
import { VscHome, VscHistory, VscLaw, VscCreditCard, VscSearch } from "react-icons/vsc";
...
function Adote() {
    ...
    return (
        <div>
            <Navbar />
            <SearchStyle>
                <input className='search' value={query} placeholder='Pesquise um nome...' onChange={(e) => { setQuery(e.target.value); setPageNumber(0);}} />
                <VscSearch />
            </SearchStyle>
            <AdoteWrapper>
                { loading ?
                    filteredPets.slice(pageVisited, pageVisited + userPerPage).map(({ _id, name, estado, idade, peso, url }) => (
                        <CardStyle key={_id}>
